...
- `doi`/`title`/`year`/`authors` strengthen **source identification** for the S2
  ...
  references are searched one by one. With an OpenAlex key, OpenAlex first and S2 last with
  `s2_retry_attempts = 1` (one request, no backoff, reference lists included) keeps keyless
  S2's few extra hits without making a run wait; leaving it out of the order drops it
  entirely. A provider whose lookups end in 429 three times in a row is skipped for
  `[citation] provider_cooldown_s` (default 10 min).
...
- Resolution logs its progress every 50 references and at the end, with the verified count
  ...
  4xx, timeout, other errors) is logged as a warning with the provider's message (contact
  address and keys masked), so an exhausted OpenAlex budget is named when it happens; later
  ones are counted in the progress lines. A 404 is counted as `missing`, not as a failure:
  it means the provider does not know that DOI or id.
