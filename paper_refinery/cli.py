...
def _refine(pdf: Path, out: Path, image_dir: Path, cfg: RefineryConfig) -> int:
...
def main(pdf: Path, out: Path | None, image_dir: Path | None) -> None:
    ...
    if image_dir is not None:
        n = _refine(pdf, out, image_dir, cfg)
    else:
        with tempfile.TemporaryDirectory(prefix="refinery-") as td:
            n = _refine(pdf, out, Path(td), cfg)

    click.echo(f"wrote {n} chunks -> {out}")
